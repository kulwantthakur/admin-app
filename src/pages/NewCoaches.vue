...
<div>
  ...
  <h6 style="margin:0!important;">New Coaches</h6>
    </div>
    <div class="col">
    </div>
    </div>
         ...
         <div>
           <q-btn style="float: right; margin-top: -40px; margin-right: 35px;" label="+ Add Coach" type="add" color="primary" @click="addcoach()" />
         </div>
          ...
          <template v-slot:top-right="props">
            ...
            >
              <template v-slot:append>
               ...
               <q-select 
                outlined v-model="selectcoaches" :options="coachesinputarray"
                @click="filter(props)" />
                </template>
...
const currencyColumns = [
{
        ...
        { name: 'status', align: 'left', label: 'Athletes', align: 'left', field: row => row.status,
...
data () {
  ...
  mounted(){
      ...
      var Id = item.id;
      //alert(Id);
      this.$router.push({ path: `/editnewcoach/${Id}` });
    },

    viewItem(item){
      var Id = item.id;
      this.$router.push({ path: `/viewnewcoach/${Id}` });
    },
       getallcoaches(){
    ...
    addcoach()
    {
       this.$router.push({path: '/addnewcoach'});
    },    
    deleteItem(item) {
       this.$q.dialog({
        title: 'Confirm',
